-- An editor that cannot say a thing will erase it.
--
-- Two components carry rpe_source = 'inherited' and rpe_default_version = 'v1':
-- an RPE that came from effort_defaults rather than from a coach typing it. The
-- component wire named rpeSource and not rpeDefaultVersion, and the explicit
-- write did not insert the column at all.
--
-- That was harmless while nothing round-tripped components. It stops being
-- harmless the moment the Console can edit them: a coach changing the reps on a
-- session would send back everything the editor knew about, and the version it
-- knew nothing about would quietly become null — an inherited default silently
-- reclassified as no default, with no error and nothing to notice.
--
-- The carry-forward path already copied the column. Now the explicit path can
-- carry it too, so a component that passes through an editor comes out the same
-- shape it went in.
...
language sql immutable as $$
  values
    ...
    ('rpeHigh',           'canonical'),
    ('rpeSource',         'canonical'),
    -- Not typed by anyone. Carried, so an editor round trip cannot drop the
    -- provenance of an RPE it did not author.
    ('rpeDefaultVersion', 'canonical'),

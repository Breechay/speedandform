-- The pace message was unreachable.
--
-- `paceLow` is not in component_wire_keys(), so the unknown-key check caught it
-- first and answered "unknown component key(s): paceLow" — true, and useless for
-- the one mistake a caller is most likely to make, because the column called
-- pace_low does exist and sending it looks reasonable.
--
-- The specific refusal runs first now. Nothing else changes.

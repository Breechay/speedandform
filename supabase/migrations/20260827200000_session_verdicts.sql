-- A session resolves on three axes, not one.
--
-- PACE   every rep inside the band that was prescribed AT THE TIME. Session
--        versions are append-only for this reason: the 6:30 to 6:45 band was set
--        after 2026-08-25, and judging that session by it would mark an athlete
--        against an instruction she was never given.
--
-- FLOAT  the recoveries were run, not rested. The reference is the athlete's own
--        warm-up and cool-down from that same session, so nothing is invented and
--        nothing has to be tuned per athlete. This is Brice's own standard, said
--        about Jose: "the floats were basically your warm up / easy pace, that's a
--        really great sign." A float more than 45 s/mi slower than the athlete's
--        own easy pace that day is rest. Jose's sat within 14 s; Hope's were 73 to
--        204 s slower.
--
-- EFFORT the logged RPE inside the prescribed band. Never a score, never compared
--        between athletes. A session can nail the pace and still land outside here,
--        which is the whole point.
--
-- A verdict states. It does not grade.

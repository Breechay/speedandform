...
// Reveal linked lessons, including nested disclosures, on direct links and back/forward.
function revealLesson(){let id;try{id=decodeURIComponent(location.hash.slice(1));}catch{return;}if(!id)return;const target=document.getElementById(id);if(!target)return;let parent=target;while(parent){if(parent.tagName==='DETAILS')parent.open=true;parent=parent.parentElement;}requestAnimationFrame(()=>target.scrollIntoView({block:'start'}));}
window.addEventListener('hashchange',revealLesson);
document.addEventListener('click',event=>{const link=event.target.closest('a[href^="#"]');if(!link)return;const menu=link.closest('.mobile-menu');if(menu)menu.open=false;if(link.hash===location.hash)revealLesson();});
revealLesson();
})();

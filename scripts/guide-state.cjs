...
function verify(file,html){if(concurrent.verify(file,html))return true;if(contact.verify(file,html))return true;if(sculpt.verify(file,html)||product.verify(file,html)||subsequent.verify(file,html))return true;const row=updates.get(file);if(!row)return false;assert.equal(sha(html),row.afterSha256,file+' exact Pass 4 editorial source');return true;}
const latestUpdates=new Map([...updates,...subsequent.updates,...product.updates,...sculpt.updates,...contact.updates,...concurrent.updates]);

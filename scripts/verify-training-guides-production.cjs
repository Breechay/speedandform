...
const s=require('./share-metadata.cjs'),guides=require('./training-guide-content.cjs'),{VERSION}=require('./build-training-guides.cjs');
const root=path.resolve(__dirname,'..'),out=process.argv[2]||'/tmp/training-guides-production.json';
...
function expectedDelivery(local){
 // All four actual response diffs in inspection run 35189385578 contain only
 // this one existing Netlify Pretty URL rewrite. Do not normalize other HTML.
 const source='<a href="/privacy.html#website">Privacy</a>';
 assert.equal(local.split(source).length-1,1,'exactly one known privacy link');
 return local.replace(source,"<a href='/privacy#website'>Privacy</a>");
}
(async()=>{
 ...
 assert.equal(s.sha(h),s.sha(expectedDelivery(local)),g.route+' exact published HTML after the single inspected hosting rewrite');
 report.pages.push({url:r.url,status:r.status,title:g.title,sections:g.sections.length,references:g.sources.length,version:VERSION,sourceSha256:s.sha(local),deliveredSha256:s.sha(h),hostingRewrite:'Only the existing footer privacy.html link becomes privacy; fragment and link text are preserved.'});save();}
 const privacy=await request('/privacy'),privacyHtml=await privacy.text();assert.ok(/id=["']website["']/.test(privacyHtml),'rewritten privacy link retains its actual destination');report.privacyDestination={url:privacy.url,status:privacy.status,fragment:'website'};save();
 for(const file of ['css/training-guides.css','js/training-tools.js','css/guide-foundations.css','js/guide-tools.js','css/cream-reading.css']){const r=await request('/'+file),b=Buffer.from(await r.arrayBuffer());assert.equal(s.sha(b),s.sha(fs.readFileSync(path.join(root,file))),file+' actual bytes');report.assets.push({file,status:r.status,sha256:s.sha(b)});save();}
 report.result='PASS';report.finishedAt=new Date().toISOString();save();console.log(`PASS: ${report.pages.length} live guides with exact article content, source links, metadata and structured data; ${report.assets.length} exact public assets; actual privacy-link destination.`);

 ...
 with sync_playwright() as p:
    ...
    for width in [375,390,430,768,1024,1440]:
        ...
        for dest,name in [('/track/','index'),(route,'album')]:
            ...
            check(f'{name}: no unnecessary instructions at {width}',not any(t in page.locator('body').inner_text() for t in ['No signup needed.','Downloads are web editions','Silent film. Play to watch.']))
        ...
        if width in [390,1440]:
            ...
            page.locator('.viewer-stage img').evaluate('(i)=>i.decode()');page.evaluate('()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)))');page.screenshot(path=str(OUT/f'viewer-{width}.png'))
    ...
    page.wait_for_function("document.activeElement.dataset.frame==='beside-the-track'");check('Focus returns to initiating image',page.evaluate("document.activeElement.dataset.frame==='beside-the-track'"))
    ...
    check('Player uses selected thumbnail',page.locator('video').get_attribute('poster')==next(x['preview']['url'] for x in album['media'] if x['type']=='video'))

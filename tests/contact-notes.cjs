...
for(const row of [...pages,...state.manifest.assets,...state.manifest.integration]){const now=read(row.file);if(!current.verify(row.file,now))state.verify(row.file,now);if(row.beforeSha256)assert.equal(s.sha(old(row.file)),row.beforeSha256,row.file+' original source');}
const changed=new Set(pages.map(p=>p.file));let preserved=0;
for(const f of cp.execFileSync('git',['ls-tree','-r','--name-only',state.manifest.baseCommit],{cwd:ROOT,encoding:'utf8'}).trim().split('\n').filter(f=>f.endsWith('.html')&&!changed.has(f))){if(!current.verify(f,read(f))){assert.equal(s.sha(read(f)),s.sha(old(f)),f+' protected HTML');preserved++;}}
for(const f of ['netlify.toml','robots.txt','_redirects','css/cream-reading.css','css/homepage.css','js/coaching-measurement.js','js/coaching-choice.js','js/homepage-motion.js','css/form-landing.css','js/form-landing.js','css/sculpt-landing.css','js/sculpt-landing.js','track/albums.json','track/media-manifest.json','css/track-gallery.css','js/track-gallery.js','js/guide-tools.js','js/training-tools.js','plans/race-pace-durability/source.js','forge-sculpt/train/data/forge-portal-programs.json'])if(!current.verify(f,read(f)))assert.equal(s.sha(read(f)),s.sha(old(f)),f+' not changed by publishing');

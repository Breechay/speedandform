...
ENGINE=os.environ.get('BROWSER','chromium');BASE='https://form-access.test'
...
try:
 with sync_playwright() as pw:
  ...
  def context_for(role='guest',width=390,failure=None,legacy=False):
   ctx=browser.new_context(viewport={'width':width,'height':960},reduced_motion='reduce');ctx.add_init_script(FIX_DATE)
  ...
  for failure in ['network','partial','contradictory']:
   ...
   check(failure+': no leaked prescription','SYNTHETIC W5' not in page.locator('body').inner_text());page.screenshot(path=str(OUT/f'{failure}-390.png'));ctx.close()
  ctx,page,seen=context_for('stranger',legacy=True);open_plan(page)
  check('Signed-in stranger does not inherit guest purchase',not any(path.endswith('rpd-entitlement') for path,_ in seen));ctx.close()
  for fail in [None,'guest-purchase']:
   ctx,page,seen=context_for('guest',legacy=True,failure=fail);page.goto(BASE+'/plans/race-pace-durability/')
   if fail:
    page.locator('#rpdRetry').wait_for();check('Failed guest purchase does not silently become preview',not any(path.endswith('public_plan_preview') for path,_ in seen))
   else:
    page.wait_for_function("document.documentElement.dataset.rpdEntitled==='true'");check('Verified guest purchase retained',True)
   ctx.close()

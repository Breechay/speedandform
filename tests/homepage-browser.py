...
with sync_playwright() as p:
 ...
 for w,h in [(320,760),(375,812),(390,844),(430,932),(768,1024),(820,1180),(1024,768),(1440,1000)]:
  ...
  assert not errors
  assert pg.locator('.analysis-screen img').count()==1
  assert pg.locator('#analysisVideo').count()==0 and pg.locator('#analysisToggle').count()==0
  passed(f'{w}px closed/open disclosures, no overflow, static coaching photo')
  pg.close()
 ...
 for outcome in ['success','rejected','network','timeout']:
  ...
  pg.screenshot(path=str(OUT/f'intake-{outcome}-390.png'),full_page=False)
  pg.close()
 pg=load(b,videos=True,motion='reduce')
 assert pg.locator('.analysis-screen img').is_visible()
 assert pg.locator('#analysisVideo').count()==0 and pg.locator('#analysisToggle').count()==0
 pg.evaluate('scrollTo(0,0)');pg.wait_for_timeout(200)
 assert pg.evaluate('filmA.paused')
 assert pg.locator('.film-toggle').is_visible()
 pg.locator('.film-toggle').click();pg.wait_for_timeout(500);assert pg.evaluate('!filmA.paused')
 pg.locator('.film-toggle').click();pg.wait_for_timeout(100);assert pg.evaluate('filmA.paused')
 passed('Static coaching photo; hero film remains manually controllable under reduced motion')
 pg.close()
 pg=load(b,videos=True,motion='no-preference');pg.wait_for_timeout(1000);assert pg.evaluate('!filmA.paused')
 pg.locator('.analysis-screen img').scroll_into_view_if_needed();pg.wait_for_timeout(1000);assert pg.evaluate('filmA.paused')
 pg.evaluate('scrollTo(0,0)');pg.wait_for_timeout(1300);assert pg.evaluate('!filmA.paused')
 pg.emulate_media(reduced_motion='reduce');pg.wait_for_function('filmA.paused',timeout=3000)
 passed('Hero film pauses offscreen and on reduced-motion preference change');pg.close()

...
for(const x of before.untouchedHtml){if(x.file==='form/index.html')x.sha256=state.manifest.pages[0].afterSha256;if(sculpt.updates.has(x.file))x.sha256=sculpt.updates.get(x.file).afterSha256;}assert.deepEqual(after,before,'Only product checksum refreshes discovery manifest');

...
report={'engine':engine,'checks':[],'errors':[],'mode':'actual anonymous production, no mocked API'}
...
try:
 with sync_playwright() as p:
  ...
  for width in [390,768,1440]:
   ...
   page.goto(ORIGIN+'/plans/race-pace-durability/')
   ...
   if width==390:
    ...
    check('Actual next-week action respects public offer boundary','/support/' in page.url)
   page.goto(ORIGIN+'/');page.wait_for_function("document.documentElement.dataset.formAccount==='signed-out'")
   check(f'{width}: anonymous home retains Sign in',page.locator('[data-form-account]').first.inner_text()=='Sign in →')
   context.close()
  check('No unexpected page errors',not report['errors'])
  browser.close()
 report['result']='PASS'

 ...
 with sync_playwright() as p:
  ...
  for width in [390,1440]:
   ...
   for route in ['easy-run','threshold-training','long-run-pace','running-form-errors']:
    ...
    page.locator('.guide-ref').first.click();page.wait_for_function("document.querySelector('#sources details').open");check(f'{route}: citation reveals sources at {width}',page.locator('#sources details').evaluate('(d)=>d.open'))
  page.goto(BASE+'/running-form-errors#hip-collapse');check('Published old fragment reaches open disclosure',page.locator('#hip-collapse').evaluate('(d)=>d.open'))

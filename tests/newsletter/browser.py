...
try:
 with sync_playwright() as pw:
  for engine in ['chromium','webkit']:
   ...
   sent_before=len([r for r in traffic if r.method=='POST']);page.goto(BASE+'/updates/#confirm='+confirm);page.wait_for_url(BASE+'/updates/');page.locator('#token-action').wait_for(state='visible')
   ...
   page.goto(BASE+'/updates/#unsubscribe='+unsubscribe);page.wait_for_url(BASE+'/updates/');page.locator('#token-action').wait_for(state='visible');check(engine+': opt-out requires one explicit page action',page.locator('#token-button').inner_text()=='Unsubscribe');page.locator('#token-button').click();page.locator('#result').wait_for(state='visible');check(engine+': unsubscribed state',page.locator('#result-title').inner_text()=='You’re unsubscribed.')
   page.screenshot(path=str(OUT/f'unsubscribed-{engine}-390.png'),full_page=True)
   page.goto(BASE+'/updates/#confirm='+confirm);page.wait_for_url(BASE+'/updates/');page.locator('#token-action').wait_for(state='visible');page.locator('#token-button').click();page.locator('#new-link').wait_for(state='visible');check(engine+': old confirmation cannot rejoin',page.locator('#result').is_hidden())
   page.goto(BASE+'/updates/#confirm=invalid');page.wait_for_url(BASE+'/updates/');page.locator('#new-link').wait_for(state='visible');check(engine+': malformed token has recovery',page.locator('#token-button').is_hidden() and page.locator('#new-link').is_visible())

...
**Updated:** September 15, 2026, after Creative A publication and RPD landing-page QA  
...
- no missing image;
- no duplicate restore link after the follow-up code cleanup.

Mobile/physical-device visual acceptance is still open.
...
## A6. Stripe / ownership — CODED, REAL-MONEY ACCEPTANCE OPEN
...
- redirect: `/plans/race-pace-durability/thanks/?session_id={CHECKOUT_SESSION_ID}`
...
5. thanks page verifies it;
6. Weeks 5–15 unlock;
7. refresh does not duplicate Purchase measurement.

Then test restore on a clean second browser/device.

## A7. Measurement — BUILT; ACCEPTANCE OPEN
...
- UTM persistence into checkout URL

Current GA4 connector read on Sep 15 showed only generic `page_view`, `first_visit`, and `session_start`; no accepted RPD purchase sequence yet.

Before meaningful spend verify visibly:
- ViewContent;
- begin checkout / InitiateCheckout;
- Purchase exactly once with $79;
- UTM path.
...
Authenticated Unbounce audit remains blocked only by login state. TinyFish reached Unbounce but the browser profile was not authenticated.
...
- [ ] Meta/GA4 pre-purchase events visibly accepted
- [ ] controlled real purchase succeeds
- [ ] Purchase event fires once at $79
- [ ] cross-device restore succeeds
- [ ] decide whether Creative B must ship before first spend or can follow A
...
Take the first unblocked item and move on if a human-only blocker appears:

1. Check latest production deploy and run phone-width/visual QA for RPD sales + preview pages.
2. Check Creative A review/status through Meta when connected data exposes it; keep parents paused meanwhile.
3. Verify pre-purchase analytics where possible without generating fake purchase evidence.
4. If Unbounce is logged in, run authenticated RPD audit and implement only truthful/high-value improvements.
5. Build Creative B if it materially improves the first test; otherwise record deliberate deferral.
6. Prepare exact real-purchase acceptance checklist and ask Brice for approval before any $79 charge.
7. After approval, test purchase → webhook → entitlement → unlock → event → restore.
8. Run final Meta preflight.
9. Enable Test 01 only when launch gate is green.
10. Then park Unbounce ideas for FORM, Forge, and coaching challenger and continue watching the Miami control.

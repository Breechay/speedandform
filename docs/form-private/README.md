# FORM Private — village operating docs

Constitutional docs for the curated Miami social village (FORM Private / Sunday Village).

| File | Role |
|------|------|
| `FORM_PRIVATE_DOCTRINE_V3.md` | Core doctrine |
| `FORM_PRIVATE_MEMBERSHIP_ARCHITECTURE.md` | Membership model |
| `FORM_PRIVATE_COORDINATION_OS_V2.md` | Coordination |
| `FORM_PRIVATE_RITUAL_ARCHITECTURE_V2.md` | Ritual structure |
| `FORM_PRIVATE_STEWARDSHIP_OS_V2.md` | Stewardship |
| `FORM_PRIVATE_AGENT_PROMPT_V5.md` | Agent pressure-test prompt |
| `The_Village_Sunday_Doctrine.pdf` | Sunday doctrine (PDF) |
| `The_Village_Creative_Layer_v2.pdf` | Creative layer (PDF) |
| `prototypes/` | Early Swift UI experiments — reference only |

**Live surface:** `village-intake.html` in speedandform repo root (canonical).

**Media:** Village shoots → `~/Documents/FORM/HIDEOUT/2026-06/`

# FORM — homepage handoff

Open `index.html` in Chrome. Phone is the real test.

Three plates, Begin, five questions, receipt, send to Brice.

Fonts load from Google (Fraunces + JetBrains Mono). Needs network once.

## Files

| File | What |
|---|---|
| `index.html` | Three plates + full questionnaire |
| `home.css` | Tokens, plates, intake |
| `home.js` | Snap, film handoff, 03 question, Begin, send |
| `media/run-development.mp4` | Plate 01 — colour film |
| `media/run-development.jpg` | Plate 01 poster |
| `media/practice.mp4` | Plate 02 / 03 — grey film, ~9s |
| `media/practice.jpg` | Plate 02 poster |
| `media/note-001.jpg` | Hope — `/notes` page frame |
| `og/note-001.jpg` | Hope — `/notes` share card, 1200×630 |
| `notes.html` | NOTE 001. One Observation. Not a grid. |
| `docs/claude-handoff/NOTES-REGISTERS.md` | Two registers |
| `docs/claude-handoff/NOTE-001.md` | Hope Observation source |
| `docs/claude-handoff/AFTER-SEND.md` | Zelle in the reply. Coaching sentence first. No cart. |
| `docs/claude-handoff/AUDIT-rd28.md` | Review Note is not a link. |
| `docs/claude-handoff/REACHABLE.md` | Zip README. `/` and `/mockupc` have the W1 tab. Race line parked. |
| `docs/claude-handoff/INSTRUMENT-PLATE.md` | 03 is the light room. Tab, weather drift. Cloud parked. |
| `docs/claude-handoff/SURFACE-AND-WEATHER.md` | Dial is the sun. Dragon is weather. Cloud parked. No weather API. |
| `docs/claude-handoff/cloud-parked/` | Two cloud directions, assets, mocks, screens. Do not restore unless asked. |
| `docs/track-c-handoff/` | Track C for Codex. Today becomes the instrument. Landed. |
| `docs/track-e-handoff/` | Track E weather for Codex. Dragon on Today. Same treatment as live `/`. |
| `docs/claude-handoff/APP-STORE-COPY.md` | FORM iOS metadata. Paste with the build. Not site copy. |
| `docs/EXPERIENCE-BRIEF.md` | Read first |
| `docs/PROMPT-FOR-CLAUDE.md` | Paste to Claude |
| `docs/ASK-BRICE.md` | Ask before writing more copy |

Do not add a cart. Do not invent athlete content. 01 + first thesis + Begin must still work if no one reaches 03. Plate 03 is the first question. The review is a receipt. Hope is at `/notes`, not on the review. Payment is Zelle, arranged in the reply — see `AFTER-SEND.md`.

Live: https://speedandform.com/

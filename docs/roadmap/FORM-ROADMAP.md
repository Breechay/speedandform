...
| FORM Track room | PR OPEN, not deployed | New `/labs/track/` in the HYROX visual language: seven Track relationships, six named standards with set-by-set targets, threshold ladder, time trials, how a mark reads, and the Track/Plan/Coaching ladder. Sources: the authored TRACK WORKOUTS sheets and `FORM_App_Track_Ecosystem_Study_v2.html`. Every set publishes the authored per-rep target plus the authored pace band per mile and per kilometer, because athletes enter the pace into a watch and some use miles while others use kilometers. The test proves each band against its own rep target: 24 of 26 sets reconcile within 3 seconds or 5 percent. The 800 m does not, on both sheets that carry one, and is a named test exception pending Brice's ruling. Tested source `b468e68` on `codex/labs-track`; `node labs/track/track.test.cjs` passes and was proven to fail on an inflated rep count and a dead fragment. No overflow at a real 320 and 390 px viewport; desktop and narrow layouts inspected in a rendered browser. Open: Brice rules on the 800 m target-versus-pace conflict, the Nice and Easy denominator (sheet gives 6, the study shows 9) and the Death vs The Long One naming collision; physical-device review; not merged, so not live. |
...
| Thursday page | PR OPEN, not deployed | `/thursday.html` corrected for the current season. Time 5:50 to 6:30 AM in the three meta descriptions and the When row. Authored Thursdays now win over the rotation: September 17 Speed Demons, September 24 Gauntlet, October 1 optional. An optional morning names no session and does not consume a rotation slot, so the session after Gauntlet is Speed Demons whichever Thursday it lands on. Removed: the stale full-week block including the Saturday Hideout line, the April race-week and recovery states, and the Key Biscayne race-week content. "Tonight" corrected to "This morning" for a 6:30 AM session. Tested source on `codex/thursday-current`; `node tests/thursday-schedule.cjs` renders the page's own script on given dates and was proven to fail on a wrong time, a session invented for the optional morning and a burned rotation slot. Open: the exact wording of the optional-morning line is Brice's to overwrite, and the sharing gate is Brice confirming details before Thursday September 17. Not merged, so not live. |

...
| FORM Track room | LIVE | `/labs/track/` published September 15, 2026 at 00:12 UTC. PR #113 squashed to main `29d90b3`; Netlify request id `01M2H6HCN0D1301PSSC9CFR3RG` on the verified response. Production verified by fetch, not assumed: the page returns 200, carries all six standards, 26 sets each with both pace units, and no athlete name other than the coach; `/labs/track/` appears in the live sitemap and on the live Labs index. Seven Track relationships, six named standards with set-by-set targets and authored pace bands per mile and per kilometer, threshold ladder, time trials, how a mark reads, and the Track/Plan/Coaching ladder. Sources: the authored TRACK WORKOUTS sheets and `FORM_App_Track_Ecosystem_Study_v2.html`. `node labs/track/track.test.cjs` passes and was proven to fail on an inflated rep count, a dead fragment and an inconsistent pace band. **Corrected after publishing.** The first live version wrote the ecosystem study's design intent in the present tense, so the page claimed capability the app does not have: "Sign in to save the mark", "Nothing is gated behind an account", "Every attempt stays", "keep the result". That is the promise-no-unverified-app-feature rule in AGENTS.md, and it contradicted the Track-in-the-app row directly below. Fixed and redeployed: the hero now states plainly that the sheets are real and the room that keeps marks is not built, section 01 is labelled as unshipped intent and written in the conditional, and section 05 describes how a mark reads rather than a list the athlete does not have. `track.test.cjs` now fails on any of the eight specific claims and requires the intent labels to be present. **Open, and live while open:** the 800 m target-versus-pace conflict on the two sheets that carry one (band implies 3:01 to 3:06, target says 2:25 to 2:30); the Nice and Easy denominator (sheet sums to 6, the study shows 9); the Death versus The Long One naming collision. Brice authorized publishing with these open. Physical-device review remains open. |

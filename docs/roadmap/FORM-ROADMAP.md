...
| FORM Track room | PR OPEN, not deployed | New `/labs/track/` in the HYROX visual language: seven Track relationships, six named standards with set-by-set targets, threshold ladder, time trials, how a mark reads, and the Track/Plan/Coaching ladder. Sources: the authored TRACK WORKOUTS sheets and `FORM_App_Track_Ecosystem_Study_v2.html`. Per-rep targets published; the sheets' per-mile and per-kilometer lines are not, because the 800 m and 150 m conversions do not follow from their own rep targets. Tested source `ae43cf3` on `codex/labs-track`; `node labs/track/track.test.cjs` passes and was proven to fail on an inflated rep count and a dead fragment. No overflow at a real 320 and 390 px viewport; desktop and narrow layouts inspected in a rendered browser. Open: Brice settles the pace-conversion question, the Nice and Easy denominator (sheet gives 6, the study shows 9) and the Death vs The Long One naming collision; physical-device review; not merged, so not live. |

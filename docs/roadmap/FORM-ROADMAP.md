...
| --- | --- | --- |
| Website ad measurement | READY FOR RELEASE | Pixel `147659485878240`; homepage PageView and Lead only after successful mail-relay response. No intake fields sent; production host and GPC/DNT guards. `node tests/coaching-measurement.cjs` passes on tracking working tree based on `709eea2`. Deployment and Meta Test Events receipt pending. Next: verify release, then owner submits one test inquiry in Events Manager. |
